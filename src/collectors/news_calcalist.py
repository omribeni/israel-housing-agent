"""
Collector for Calcalist real-estate news articles.

Scrapes the Calcalist real-estate section and extracts article metadata
...
_BASE_URL = "https://www.calcalist.co.il"
_SECTION_URL = f"{_BASE_URL}/real-estate"
_MAX_ARTICLES = 20


class CalcalistCollector(BaseCollector):
    """Collects housing-related articles from Calcalist's real-estate section."""
    ...
    async def collect(self) -> list[RawArticle]:
        """Fetch the Calcalist real-estate listing page and extract articles."""
        try:
            html = await fetch_page(_SECTION_URL, self._settings)
            return self._parse(html)
        except Exception:
            logger.exception("CalcalistCollector failed to collect articles")
            return []
    ...
    def _parse(self, html: str) -> list[RawArticle]:
        ...
        logger.info("CalcalistCollector parsed %d articles", len(articles))
        return articles

    @staticmethod
    def _is_article_url(href: str) -> bool:
        """Return True if the href looks like a Calcalist article link."""
        # Calcalist articles typically live under /article/ or contain
        # numeric article IDs in the path.
        if "/article/" in href:
            return True
        if "/real-estate/" in href or "/real_estate/" in href:
            parts = href.rstrip("/").split("/")
            if len(parts) >= 3:
                return True
        # Some Calcalist articles use /calcalistech/ or direct numeric slugs
        if "/articles/" in href:
            return True
        return False
